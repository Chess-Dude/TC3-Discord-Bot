...
class ChannelUtilityCommands(commands.Cog):
    ...
    @app_commands.checks.has_any_role(554152645192056842, 743302990001340559, 351074813055336458, 475669961990471680)
    @create_command_group.command(
        name="one_day_1v1",
        description="A Command That Allows You To Create A Server Channel!")
    @app_commands.describe(player_1="P1ing a member to be added to the channel here!")
    @app_commands.describe(player_2="Ping a member to be added to the channel here!")
    @app_commands.rename(player_1="player_1")
    @app_commands.rename(player_2="player_2")
    async def channel_create_1v1_tourney(        
        self,
        interaction: discord.Interaction,
        player_1: discord.Member,
        player_2: typing.Optional[discord.Member]
    ):
        ec_role = interaction.guild.get_role(475669961990471680)
        mod_role = interaction.guild.get_role(351166789700550679)
        mit_role = interaction.guild.get_role(363125947635073025)
        ...
        overwrites = {
            interaction.guild.default_role: discord.PermissionOverwrite(read_messages=False),
            interaction.guild.me: discord.PermissionOverwrite(read_messages=True),
            player_1: discord.PermissionOverwrite(read_messages=True, send_messages=True),
            player_2: discord.PermissionOverwrite(read_messages=True, send_messages=True),
            ec_role: discord.PermissionOverwrite(read_messages=True, send_messages=True),
            mod_role: discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_messages=True),
            mit_role: discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_messages=True)
        }
        ...
        new_channel = await interaction.guild.create_text_channel(
            name=f"{player_1.name}-vs-{player_2.name}", 
            overwrites=overwrites, 
            category=category
        )
    ...
    @app_commands.checks.has_any_role(554152645192056842, 351074813055336458, 475669961990471680)
    @delete_command_group.command(
        name="one_day_channel",
        description="A Command That Allows You To Delete A Server Channel!")
    @app_commands.describe(channel_1="mention a channel to be deleted")
    @app_commands.describe(channel_2="mention a channel to be deleted")
    @app_commands.describe(channel_3="mention a channel to be deleted")
    ...
    @app_commands.rename(channel_4="channel_4")
    @app_commands.rename(channel_5="channel_5")            
    async def channel_delete_one_day(
        self,
        interaction: discord.Interaction,
        channel_1: discord.TextChannel,
        channel_2: typing.Optional[discord.TextChannel],
        ...
        for channel in channel_list: 
            ...
            (channel.id not in immune_channel_ids)):
                if channel.category == contest_category or channel.category == tournament_category:  
                    ...
                    await interaction.channel.send(f"Could not delete {channel} as it does not belong in the 'one-day tournament' category.")
            else:
                break
