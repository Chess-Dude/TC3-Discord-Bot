import discord, datetime, asyncio, typing
...
class ModeratorCommands(commands.Cog):
    ...
    @app_commands.command(
        name="unmute",
        description="A Command that allows moderators to unmute a member")
    ...
    async def appeal(
        ...
        async with self.bot.pool.acquire() as connection:
            ...
            user_data = await connection.fetchrow("""
                SELECT activeDamage, healedDamage
                FROM UserTotalModlogs
                WHERE userID = $1 AND guildID = $2
            """, user.id, interaction.guild.id)

            if not user_data:
                await interaction.response.send_message("User not found in modlogs.", ephemeral=True)
                return

            # Convert current values to Decimal
            active_decimal = Decimal(str(user_data["activedamage"]))
            healed_decimal = Decimal(str(user_data["healeddamage"]))
            remove_decimal = Decimal(str(remove_damage))
            
            # Calculate new values
            new_active = max(Decimal('0.0'), active_decimal - remove_decimal)
            new_healed = healed_decimal + remove_decimal
            ...
            """, float(new_active), float(new_healed), user.id, interaction.guild.id)

            # Send confirmation
            await interaction.response.send_message(
                f"✅ Successfully appealed {remove_damage} damage for {user.mention}.\n"
                f"New active damage: {float(new_active)}\n"
                f"New healed damage: {float(new_healed)}",
                ephemeral=True
            )
